use crate::{
    Shape, ShapeExt, bytes::StoreBytes, pipeline::create_pipeline, primitive::VertexData,
...
struct PolygonContextKey;
impl RegistryKey for PolygonContextKey {
    type Value = PolygonContext;
}
impl RegistryInit for PolygonContextKey {
    fn create_value(&self, state: &Graphics) -> Self::Value {
        PolygonContext::new(state)
    }
}
...
impl PolygonContext {
    pub fn new(state: &Graphics) -> Self {
        ...
        let pipeline =
            create_pipeline(state, &Default::default()).expect("Error creating pipeline");
