...
pub trait Shapes3d {
    /// Unit-radius Z-up sphere with smooth normals and equirectangular UVs.
    /// At least 3 sectors and 2 rings.
    fn sphere(&self, sectors: u32, rings: u32) -> Polygon;
    /// Closed unit-radius cylinder, Z from -0.5 to 0.5, with sharp cap edges.
    /// Caps map a disk to the texture; the side maps circumference to U and
    /// height to V. At least 3 sectors.
...
fn vertex(pos: Vec3, uv: Vec2) -> Vertex {
    Vertex::new(pos.extend(1.0), uv.extend(1.0)).with_normal(pos.normalize_or_zero())
}
...
pub fn cylinder_mesh(sectors: u32) -> (Vec<Vertex>, Vec<u32>) {
    ...
    for z in [-0.5, 0.5] {
        ...
        for i in 0..=sectors {
            ...
            vertices
                .push(vertex(p.extend(z), (p + Vec2::ONE) * 0.5).with_normal(Vec3::Z * z.signum()));
            if i < sectors {
    ...
    for i in 0..=sectors {
        ...
        for (z, v) in [(-0.5, 1.0), (0.5, 0.0)] {
            vertices.push(vertex(p.extend(z), Vec2::new(u, v)).with_normal(p.extend(0.0)));
        }
        if i < sectors {
            let a = offset + 2 * i;
